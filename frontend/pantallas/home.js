...
import { View, Image, Text, StyleSheet, TouchableOpacity } from 'react-native';
import * as ImagePicker from 'expo-image-picker';
import { predictImage } from '../config/api';  // Funció api centralitzada
import { useTheme } from '../config/contextoEstilo';
import { SafeAreaView } from 'react-native-safe-area-context';
import { MaterialIcons } from '@expo/vector-icons';
import { diagnosticMap } from '../data/diagnostics';
import { useRefresh } from '../config/RefreshContext';
import { API_BASE } from '../config/ip';
import { AuthContext } from '../config/AuthContext';  // Importem el context d’autenticació
...
export default function HomeScreen() {
  ...
  const [result, setResult] = useState(null);
  const { setRefreshNeeded } = useRefresh();
  const { logout } = useContext(AuthContext);  // Podem usar logout per quan hi ha 401
